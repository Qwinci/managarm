...
struct BatteryBusObject final : public KernelBusObject {
	BatteryBusObject(size_t id, qacpi::NamespaceNode *node)
...
private:
	...
	static void notification(void *, qacpi::NamespaceNode *node, uint64_t value);
...
namespace BIF {
	namespace PowerUnit {
		constexpr uint32_t MILLIWATT = 0;
		constexpr uint32_t MILLIAMPERE = 1;
	} // namespace PowerUnit
	...
	namespace BatteryTechnology {
		constexpr uint32_t PRIMARY = 0;
		constexpr uint32_t SECONDARY = 1;
	} // namespace BatteryTechnology
...
namespace BST {
	namespace State {
		constexpr uint32_t DISCHARGING = (1 << 0);
		constexpr uint32_t CHARGING = (1 << 1);
		constexpr uint32_t CRITICAL_ENERGY_STATE = (1 << 2);
		constexpr uint32_t CHARGE_LIMITING = (1 << 3);
	} // namespace State
...
coroutine<void> BatteryBusObject::run() {
	...
	globalEventCtx->install_notify_handler(_node, BatteryBusObject::notification, this);
}
...
void BatteryBusObject::notification(void *arg, qacpi::NamespaceNode *node, uint64_t value) {
	if(logBatteryNotifications) {
		auto path = node->absolute_path();
		infoLogger() << "thor: battery '" << path.data() << "' received AML Notify(" << value << ")" << frg::endlog;
	}

	auto self = static_cast<BatteryBusObject *>(arg);
...
void BatteryBusObject::updateBIF() {
	auto bif = qacpi::ObjectRef::empty();

	auto ret = globalCtx->evaluate_package(_node, "_BIF", bif);
	if(ret != qacpi::Status::Success) {
		infoLogger() << "thor: _BIF error " << qacpi::status_to_str(ret) << frg::endlog;
		return;
	}

	auto power_unit = intFromPackageQacpi(bif, 0);
	auto design_capacity = intFromPackageQacpi(bif, 1);
	auto last_full_charge_capacity = intFromPackageQacpi(bif, 2);
	auto battery_technology = intFromPackageQacpi(bif, 3);
	auto design_voltage = intFromPackageQacpi(bif, 4);
...
void BatteryBusObject::updateBST() {
	auto bst = qacpi::ObjectRef::empty();

	auto ret = globalCtx->evaluate_package(_node, "_BST", bst);
	if(ret != qacpi::Status::Success) {
		infoLogger() << "thor: _BST error " << qacpi::status_to_str(ret) << frg::endlog;
		return;
	}

	auto battery_state = intFromPackageQacpi(bst, 0);
	auto present_rate = intFromPackageQacpi(bst, 1);
	auto remaining_capacity = intFromPackageQacpi(bst, 2);
	auto present_voltage = intFromPackageQacpi(bst, 3);
	...
	if(_state.acpi_units && present_rate && *present_rate != BST::Rate::UNKNOWN) {
		if(*_state.acpi_units == BIF::PowerUnit::MILLIAMPERE) {
			_state.rate_milliampere = frg::abs(int32_t(*present_rate));
...
void initializeBatteries() {
	async::detach_with_allocator(*kernelAlloc, []() -> coroutine<void> {
		...
		qacpi::EisaId batteryId{"PNP0C0A"};
		globalCtx->discover_nodes(nullptr, &batteryId, 1, [](qacpi::Context &, qacpi::NamespaceNode *node) {
			auto bif = globalCtx->find_node(node, "_BIF");
			auto bst = globalCtx->find_node(node, "_BST");
