	...
	coroutine<frg::expected<Error>> handleRequest(LaneHandle lane) override {
		...
		if(preamble.id() == bragi::message_id<managarm::hw::RebootRequest>) {
			...
			switch(req->cmd()) {
				case RB_POWER_OFF: {
#ifdef __x86_64__
					auto ret = globalEventCtx->prepare_for_sleep_state(*globalCtx, qacpi::events::SleepState::S5);
					if(ret != qacpi::Status::Success)
						infoLogger() << "thor: Preparing to enter sleep state S5 failed: " << qacpi::status_to_str(ret) << frg::endlog;

					disableInts();

					ret = globalEventCtx->enter_sleep_state(qacpi::events::SleepState::S5);
					if(ret != qacpi::Status::Success)
						infoLogger() << "thor: Entering sleep state S5 failed: " << qacpi::status_to_str(ret) << frg::endlog;
#endif
				...
				case RB_AUTOBOOT: {
					...
					auto ret = globalEventCtx->prepare_for_sleep_state(*globalCtx, qacpi::events::SleepState::S5);
					if(ret != qacpi::Status::Success)
						infoLogger() << "thor: Preparing for reboot failed: " << qacpi::status_to_str(ret) << frg::endlog;

					disableInts();

					ret = globalEventCtx->reboot();
					if(ret != qacpi::Status::Success)
						infoLogger() << "thor: ACPI reset failed: " << qacpi::status_to_str(ret) << frg::endlog;
